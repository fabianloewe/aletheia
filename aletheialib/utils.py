...
def check_bin(cmd):
    ...
    if not shutil.which(cmd):
        click.echo("ERROR: you need to install " + cmd + " to run this command!");
        sys.exit(0)
...
def download_octave_code(method):
    ...
    try:
        ...
        click.echo(f"Error, {method} license cannot be downloaded")
        sys.exit(0)

    # Accept license
    click.echo("\nCODE:", method)
    click.echo("\nTo proceed, kindly confirm your acceptance of the license and your")
    click.echo("agreement to download the code from 'aletheia-external-resources'\n")
    ...
    while r not in ["y", "n"]:
        ...
        if r == "n":
            click.echo("The terms have not been accepted\n")
            sys.exit(0)
        elif r == "y":
            break
    ...
    try:
        ...
        click.echo(f"Error, {method} code cannot be downloaded")
        sys.exit(0)
...
def download_octave_aux_file(fname):
    ...
    try:
        ...
        click.echo(f"Error, {fname} cannot be downloaded")
        sys.exit(0)
...
def download_octave_jpeg_toolbox():
    ...
    try:
        ...
        click.echo("Error, JPEG TOOLBOX license cannot be downloaded")
        sys.exit(0)

    # Accept license
    click.echo("\nTo use this command, you must accept the license of the JPEG TOOLBOX. In that")
    click.echo("case, we will download the JPEG TOOLBOX from 'aletheia-external-resources'.\n")
    with open(local_license_file, 'r') as f:
        click.echo(f.read())
    ...
    while r not in ["y", "n"]:
        ...
        if r == "n":
            click.echo("The license has not been accepted\n")
            sys.exit(0)
        elif r == "y":
            break
    ...
    try:
        ...
        click.echo(remote_file)
        click.echo(local_file)
        click.echo("Error, JPEG_READ_STRUCT cannot be downloaded")
        sys.exit(0)
...
def download_e4s():
    ...
    try:
        ...
        click.echo("Error, e4s license cannot be downloaded")
        sys.exit(0)
